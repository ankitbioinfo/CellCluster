...
function makeClusterTifFiles(olddirectory,tile,coordinates,path,LCCall,unique_tileid,bonetype,centroid,GlobalCenter)
     ...
     for k=1:length(LCCall)
		...
		if ((position~=9) & (position~=8) & (position~=10) & (position~=14) & (position~=28))
...
%                             pos=coords(i,:);
%                             centroids = calc_centroids(masks(i), origins(i,:));
...
%                             end           
%                         end             
%                         tifname=strcat(save_folder,'/tile_',num2str(position),'_col_',num2str(highOrinetcolumnId(k)),...
%                             '_wholePC1_',num2str( wholePC1(k)),'_AvgPC3_',num2str( cellAvgPC3(k)),'.tif');
% 
% 
%                         saveTiffFiles(tifname,I,255*characteristics)  
