...
%
%        |  |  |
% - M : [Mx My Mz] double row vector -or matrix-, the point -or list of N points- which
%        |  |  |
%
%       we want the distance to the plane. size(M) = [N, 3].
%
% - n : [nx ny nz], double row vector, a plane normal vector, size(n) = [1, 3].
%
% - I : [Ix Iy Iz], double row vector, a point belonging to the
%       plane, size(I) = [1, 3].
% 
%% Output arguments
%
% - d2H : double scalar -or vector-, the euclidian distance between M and the plane (I,n). size(d2H) = [N, 1].
% - H : double vector, the projected point(s) on the plane. size(H) = [N, 3].
%
%% Example #1
% Single point
%
