...
%         end
       ...
       CurvatureOfColumn=plotClusterFigure( pureColumn,LCCall,centroid,nuc, C, bonetypeMatrix,'r.');
       ...
       [tri, V] = freeBoundary(trep);
        ...
        savefolder='Curvature/figure_colum_length/';
...
function output=plotClusterFigure(pureColumn,LCCall,centroid,csurfVertices,globalCenter, bonetypeMatrix,mycolor)
        ...
        if length(LCC)>=2
            ...
            for i=1:size(LCC,1)
             ...
             for i=1:length(pureColumn)
                 if k==pureColumn(i)
                     if length(LCC)==3
                        %smoothUsingAverage(csurf,'m.-',1);
                         plot3(mu(:,1),mu(:,2),mu(:,3),'m.-','linewidth',0.5);
                     elseif length(LCC)==4
                        %smoothUsingAverage(csurf,'k.-',1);
                         plot3(mu(:,1),mu(:,2),mu(:,3),'k.-','linewidth',1);

                     elseif length(LCC)>=5
                        %smoothUsingAverage(csurf,'g.-',1);
                         plot3(mu(:,1),mu(:,2),mu(:,3),'r.-','linewidth',2);

%                      elseif length(LCC)==6
%                         %smoothUsingAverage(csurf,'r.-',1);
%                          plot3(mu(:,1),mu(:,2),mu(:,3),'r.-','linewidth',3);
% 
%                      elseif length(LCC)==7
%                         %smoothUsingAverage(csurf,'y.-',1);
%                          plot3(mu(:,1),mu(:,2),mu(:,3),'y.-','linewidth',5);
