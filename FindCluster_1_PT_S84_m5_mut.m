...
%         nuccent=nuc(:,5:7);
 ...
 makeClusterTifFiles(input2,tile,coordinates,path,LCCall,unique_tileid,unique_pixel,vec,bonetype,centroid,  GlobalCenter{gi}{gj});
...
function makeClusterTifFiles(olddirectory,tile,coordinates,path,LCCall,unique_tileid,unique_pixel,vec,bonetype,centroid,GlobalCenter)
...
%                     end       
                       ...
                       for i=1: size(C.coords,1)
                            ...
                            centroids = (calc_centroids_spacing(C.masks(i), C.origins(i,:), C.spacing(i,:)) +repmat([tile(fi,2), -tile(fi,1),tile(fi,3)],1,1))*vec;  
                          
                            for j=1:length(LCC)
                                  if fi==unique_tileid(LCC(j),1)
                                        %k=unique_tileid(LCC(j),2);
                                        %predicted_cent=unique_pixel(LCC(j),:);
                                        predicted_cent = centroid(LCC(j),:);
                                        dist=pdist([predicted_cent;centroids]);
                                        if dist<1
                                            %[k,i,LCC(j),centroids,predicted_cent]
                                            characteristics(pos(1):pos(2),pos(3):pos(4),pos(5):pos(6))= characteristics(pos(1):pos(2),pos(3):pos(4),pos(5):pos(6)) | C.masks{i};
                                        end
                                  end
                            end           
...
function centroids = calc_centroids_spacing(masks, origins, spacings)
